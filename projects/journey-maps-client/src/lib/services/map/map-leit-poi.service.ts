...
export class MapLeitPoiService {
  ...
  processData(map: maplibregl.Map, featureCollection: GeoJSON.FeatureCollection = this.mapService.emptyFeatureCollection): void {
    this.removeMapLeitPois();
  ...
  setCurrentLevel(map: maplibregl.Map, currentLevel: number): void {
    this.showLeitPoiByLevel(map, currentLevel);
  }

  private registerMapZoomEvent(map: maplibregl.Map): void {
  ...
  private showLeitPoiByLevel(map: maplibregl.Map, currentLevel: number): void {
    this.removeMapLeitPois();
    this.getFeaturesByLevel(currentLevel).forEach(f => this.showLeitPoi(map, f));
    this.toggleMapLeitPoisVisibility(map.getZoom());
  }

  private showLeitPoi(map: maplibregl.Map, feature: LeitPoiFeature): void {
